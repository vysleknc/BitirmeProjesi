...
public class OrderController {
	...
	public String filterOrdersByCategory(@RequestParam MultiValueMap<String, String> filterCriteria, Model model) {
		...
		for (ProductOrder order : orders) {
			...
			if (filterCriteria.get("category").get(0).equals("Freezer")
					&& order.getProduct().getProductLine().getCategory().equals(ProductCategory.Freezer)) {
				filteredOrders.add(order);
			}
			else if (filterCriteria.get("category").get(0).equals("WashingMachine")
					&& order.getProduct().getProductLine().getCategory().equals(ProductCategory.WashingMachine)) {
				filteredOrders.add(order);
			}
		}
